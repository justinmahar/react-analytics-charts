...
This component allows you to drop in an analytics dashboard that automatically handles loading the [Google Analytics Embed API](https://react-use-analytics-api.netlify.com/), [sign in](/AuthorizeButton) and [sign out](/SignOutButton), and [view selection](/ViewSelector).
...
Just use the `renderCharts` prop to render the charts you'd like to view and the dashboard will handle everything else, including loading the [Google Analytics Embed API](https://react-use-analytics-api.netlify.app/), handling auth, and showing a view selector.
