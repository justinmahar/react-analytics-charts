...
import { SessionsByUserTypeChart } from './SessionsByUserTypeChart';

<GoogleAnalytics trackingId={TRACKING_ID} />

<h1>SessionsByUserTypeChart</h1>

## Description

This [PieChart](/PieChart) charts the number of sessions for each user type (new visitors vs. returning visitors), in the date range specified (number of days ago, until today).
...
<Playground>
  <AnalyticsDashboard
    ...
    renderCharts={(gapi, viewId) => {
      return <SessionsByUserTypeChart gapi={gapi} viewId={viewId} days={28} />;
    }}
  />
</Playground>
