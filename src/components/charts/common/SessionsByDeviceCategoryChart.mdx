...
menu: Common Charts
route: /SessionsByDeviceCategoryChart
---
...
import { SessionsByDeviceCategoryChart } from './SessionsByDeviceCategoryChart';

<GoogleAnalytics trackingId={TRACKING_ID} />

<h1>SessionsByDeviceCategoryChart</h1>

## Description

This [PieChart](/PieChart) charts the number of sessions for each device category (such as `desktop`, `mobile`), in the date range specified (number of days ago, until today).
...
<Playground>
  <AnalyticsDashboard
    ...
    renderCharts={(gapi, viewId) => {
      return <SessionsByDeviceCategoryChart gapi={gapi} viewId={viewId} days={28} />;
    }}
  />
</Playground>
...
| options | [PieChartOptions](/PieChart#chart-options) | Optional. The options for this chart have been preconfigured, but you can add or override any [PieChartOptions](/PieChart#chart-options) you'd like here, such as the `width` or `title`. |
