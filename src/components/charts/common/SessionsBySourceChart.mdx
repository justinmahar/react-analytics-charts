...
import { SessionsBySourceChart } from './SessionsBySourceChart';

<GoogleAnalytics trackingId={TRACKING_ID} />

<h1>SessionsBySourceChart</h1>

## Description

This [PieChart](/PieChart) charts the number of sessions for each source (such as `(direct)`, `google`, or a domain name), in the date range specified (number of days ago, until today).
